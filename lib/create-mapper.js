            ...
            async.parallel([function (cb) {
                fs.readFile(_dirname + '/tpl/java/service/mapper/MapperBase.java', function (err, data) {
                    async.eachSeries(entitys, function (item, callback) {
                        fs.writeFile(_dirname + '/_tmp/' + item.name + '-service' + '/src/main/java/' + item.groupid.replace(/\./g, "/") + '/' + item.name + '_service/mapper/' + tf2(item.entity) + 'Mapper.java', Handlebars.compile(data.toString())(item), function (e) {
                            callback(e);
                        });
                    }, function (er) {
                        cb(er);
                ...
                fs.readFile(_dirname + '/tpl/resources/service/mapper/MapperBase.xml', function (err, data) {
                    async.eachSeries(entitys, function (item, callback) {
                        fs.writeFile(_dirname + '/_tmp/' + item.name + '-service' + '/src/main/resources/mapper/' + tf2(item.entity) + 'Mapper.xml', Handlebars.compile(data.toString())(item), function (e) {
