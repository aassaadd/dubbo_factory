        ...
        async.eachSeries(rows, function (item, callback) {
            ...
            fs.readFile(_dirname + '/tpl/java/facade/model/BeanBase.java', function (err, data) {
                async.eachSeries(entitys, function (item, callback) {
                    fs.writeFile(_dirname + '/_tmp/' + item.name + '-facade' + '/src/main/java/' + item.groupid.replace(/\./g, "/") + '/' + item.name + '_facade/model/' + tf2(item.entity) + '.java', Handlebars.compile(data.toString())(item), function (e) {
                        callback(e);
                    });
                }, function (er) {
