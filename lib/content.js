    ...
    connectionpool.getConnection(function (err, connection) {
        if (err) {
            ...
            async.parallel([function (callback) {
                ...
                if(program.apidoc){
                    //8.创建doc文件
                    createDoc(program, _dirname, connection,callback);
                }else{
                    callback(null);
                }
                ...
                if(program.apidoc){
                    //打包apidoc
                    var doczipPath = program.src+'/apidoc.zip';
                    //创建一最终打包文件的输出流
                    var docoutput = fs.createWriteStream(doczipPath);
                    //生成archiver对象，打包类型为zip
                    var doczipArchiver = archiver('zip');

                    //将打包对象与输出流关联
                    doczipArchiver.pipe(docoutput);
                    // zipArchiver.directory(_dirname+'/_tmp',zipPath);
                    doczipArchiver.bulk([{ expand: true, cwd: _dirname+'/_doc/output', src: ['**'] }]);
                    //打包
                    function docit() {
                        if(doczipArchiver._state.finalized){
                            process.exit(0);
                        }else{
                            setTimeout(docit,1000);

                        }
                    }
                    doczipArchiver.finalize();
                    docit();
                }
            });
        }
