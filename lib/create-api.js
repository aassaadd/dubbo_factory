            ...
            async.eachSeries(rows, function (item, callback) {
                ...
                async.parallel([function (cb) {
                    fs.readFile(_dirname + '/tpl/java/api/common/System.java', function (err, data) {
                        fs.writeFile(_dirname + '/_tmp/' + program.name + '-api' + '/src/main/java/' + program.groupid.replace(/\./g, "/") + '/' + program.name + '_api/common/' + tf2(program.name) + 'System.java', Handlebars.compile(data.toString())(program), function (e) {
                            cb(e);
                        });
                    });
                }, function (cb) {
                    ...
                    fs.readFile(_dirname + '/tpl/java/api/restful/ControllerBase.java', function (err, data) {
                        async.eachSeries(entitys, function (item, callback) {
                            fs.writeFile(_dirname + '/_tmp/' + item.name + '-api' + '/src/main/java/' + item.groupid.replace(/\./g, "/") + '/' + item.name + '_api/restful/' + tf2(item.entity) + 'Controller.java', Handlebars.compile(data.toString())(item), function (e) {
                                callback(e);
                            });
                        }, function (er) {
                            cb(er);
