        ...
        async.eachSeries(rows, function (item, callback) {
            ...
            async.parallel([function (cb) {
                fs.readFile(_dirname + '/tpl/java/api/common/System.java', function (err, data) {
                    fs.writeFile(_dirname + '/_tmp/' + program.name + '-api' + '/src/main/java/' + program.groupid.replace(/\./g, "/") + '/' + program.name + '_api/common/' + program.name  + 'System.java', Handlebars.compile(data.toString())(program), function (e) {
                        cb(e);
                    });
                });
            },function (cb) {
